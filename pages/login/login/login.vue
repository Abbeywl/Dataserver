<template>
	<view>
		...
		<view class="main1">
			<u-toast ref="uToast" />
			<view class="" @click="navBack" style="margin-bottom: 20px;">
				<u-icon name="icon iconfontsb icon-arrowLeft-fill" size="60"></u-icon>
			</view>
			<view class="title_des">欢迎使用车联网络信息安全APP</view>
			<view class="item">
		...
		<view class="footer" style="margin-top: 16px">
			<view>
				<navigator url="/pages/login/register/select" open-type="navigate">新用户注册</navigator>
			</view>
			<view style="margin-left: 32%;">
				<navigator url="/pages/login/forget/person" open-type="navigate">忘记密码</navigator>
			</view>
		</view>
		...
		<view class="other_login cuIcon">
			<view class="login_icon">
				<u-icon name="icon iconfontsb icon-weixinfang" size="55" color="#1afa29"></u-icon>
			</view>
			<view class="login_icon">
				<u-icon name="icon iconfontsb icon-zhifubao" size="65" color="#1296db"></u-icon>
			</view>
			<view class="login_icon">
				<u-icon name="icon iconfontsb icon-weibo1" size="60" color="#d81e06"></u-icon>
			</view>
			<view class="login_icon">
				<u-icon name="icon iconfontsb icon-apple" size="50"></u-icon>
			</view>
		</view>
		<view class="footer_des">你的车联网信息安全专家</view>
	</view>
</template>
...
import testdata from '../../work/testdata.js';
	export default {
		...
		methods: {
				...
				this.$Request
					...
					.then(res => {
						if (res.resultCode == 1) {
							...
							if (res.data.type == 1) {
								this.$queue.setData('userName', res.data.telNumber);
								this.$queue.setData('mobile', res.data.telNumber);
							} else {
								this.$queue.setData('userName', res.data.usrMail);
								this.$queue.setData('mobile', res.data.usrMail);
							}
							this.$queue.setData('nickName', res.data.userName);
							this.loginJifen();
			...
			checkOpenid(openid) {
				this.$queue.showLoading('登录中...');
				this.$queue.setData('openid', openid);
				this.$Request
					.postF('/system/user/openId', {
						openId: openid
					})
					.then(res => {
