<template>
 ...
 -->
			<!-- <view class="title_des">企业注册</view> -->
			<view class="item">
				<image class="login_image" src="../../../static/login/userName.png"></image>
				<input class="main-input" v-model="companyNumber" placeholder="请输入企业编号" maxlength="20" />
			</view>
			<view style="margin-top: 8px"></view>
			<view class="item">
				<image class="login_image" src="../../../static/login/email.png"></image>
				<input class="main-input" v-model="email" placeholder="请输入企业邮箱" />
			</view>
			<view style="margin-top: 8px"></view>
			<view class="item">
				<image class="login_image" src="../../../static/login/code.png"></image>
				<view class="type" style="border-bottom: 1px solid #DDDDDD;">
			...
			<view class="item">
				...
				<view class="type" style="border-bottom: 1px solid #DDDDDD;"><input v-model="passData" class="main-input-code" maxlength="20" type="password" placeholder="请设置密码" /></view>
			</view>
		</view>
		...
		<view class="register_footer">
			...
			<!-- 协议地址 -->
			<navigator url="/pages/login/xieyi/xieyi" open-type="navigate">《用户服务协议》</navigator>
			和
			<navigator url="/pages/login/yingsi/yingsi" open-type="navigate">《用户隐私协议》</navigator>
		</view>
	</view>
</template>
	...
	methods: {
		...
		sendMsg() {
			if (this.email.length == 0) {
				this.$queue.showToast('请输入企业邮箱');
				return;
			}
			var reg = new RegExp('^[a-z0-9]+([._\\-]*[a-z0-9])*@([a-z0-9]+[-a-z0-9]*[a-z0-9]+.){1,63}[a-z0-9]+$'); //正则表达式
			if (!reg.test(this.email)) {
				this.$queue.showToast('请输入正确的邮箱');
				return;
			}

			this.$queue.showLoading('获取验证码中...');
			this.$Request
				.get('/system/user/getCodeRegister?sendNumber=' + this.email + '&isRegister=0')
				.then(res => {
					if (res.resultCode == 1) {
		...
		toRegister() {
			...
			this.$Request
				.postF('/system/user/register?code=' + this.code, {
					createType: 1,
