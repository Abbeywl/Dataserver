	...
	<view>
		...
		<view class="main">
			...
			<view class="item">
				...
				<input class="main-input" v-model="phoneData" placeholder="请输入手机号/邮箱" />
			</view>
		...
		methods: {
			...
			sendMsg() {
				...
				if (this.phoneData.length == 11 || this.phoneData.split('@') != -1) {
					...
					this.$Request
						.get('/system/user/getCodeRegister?sendNumber=' + this.phoneData + '&isRegister=1')
						.then(res => {
							if (res.resultCode == 1) {
			...
			toForget() {
				this.$queue.showLoading('找回中...');
				this.$Request
					.postF('/system/user/findBackPassword', {
						sendNumber: this.phoneData,
						code: this.code,
						password: this.passData
					})
					.then(res => {
						if (res.resultCode == 1) {
							this.$queue.showToast('密码找回成功');
							uni.reLaunch({
